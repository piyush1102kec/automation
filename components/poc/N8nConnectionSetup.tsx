...
import { Eye, EyeOff, CheckCircle2, XCircle, Loader2, ChevronDown, ChevronUp, Wifi } from 'lucide-react';
    ...
    try {
      ...
      if (data.connected) {
        ...
        if (data.ollama) {
          ...
          setOllamaMsg(data.ollama.ok ? data.ollama.model : (data.ollama.error ?? 'AI provider error'));
        }
      } else {
        setTestStatus('error');
            ...
            <div className="space-y-2">
              ...
              {ollamaOk === true && (
                <div className="flex items-center gap-2 text-xs text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg px-3 py-2">
                  <CheckCircle2 className="w-3.5 h-3.5 shrink-0" />
                  AI ready — <span className="font-mono font-semibold">{ollamaMsg}</span>
                </div>
              )}
              {ollamaOk === false && (
